...
const CreateSubActivityScreen = () => {
    const { accessToken, user } = useContext(AuthContext)
    const navigation = useNavigate()

    const { year, month, day, taskId } = useParams()
    ...
    const AddActivity = (e) => {
        e.preventDefault()
        fetch('http://127.0.0.1:8000/api/sub-activity/add', {
            method: "POST",
            headers: {
    ...
    return (
        <div className='CreateActivityContainer'>
            <form method='POST' onSubmit={AddActivity} className='CreateActivityForm'>
                ...
                <button className='CreateActivityBtn' type='submit'>Create SubActivity</button>          
            </form>
        </div>
    )
}

export default CreateSubActivityScreen