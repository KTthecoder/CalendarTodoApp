...
const EditSubTask = () => {
    ...
    const { year, month, day, id, taskId } = useParams()
    ...
    const EditActivityPUT = (e) => {
        e.preventDefault()
        fetch(`http://127.0.0.1:8000/api/sub-activity/edit/${id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div className='CreateActivityContainer'>
            <form method='PUT' onSubmit={EditActivityPUT} className='CreateActivityForm'>
                ...
                <button className='CreateActivityBtn' type='submit'>Edit Sub Activity</button>          
            </form>
        </div>
    )
}
